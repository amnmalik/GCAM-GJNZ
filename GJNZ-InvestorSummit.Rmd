...
theme_set(theme_pubclean(base_size = 14, base_family = "Calibri"))
```
...
# Color Palette CEEW
```{r}
my_custom_palette <- c("#009CD8", "#8DB824", "#575756", "#71C9EB","#B4D48C", "#F49A70","#9D9D9C","#C3E5F5","#DDE9BA","#FACAB1","#D5D7DC")
```



```{r eval=FALSE, include=FALSE}
## Connecting to GCAM database and writing database output to a project file
...
prj <- addScenario(conn, proj = 'GJNZ.dat', scenario =  c('BAU_GJ'), queryFile = queryFileInput,warn.empty = T,clobber = T) # This command runs queries from queryFileInput against the database, extract the results for the said scenarios called and write the results to a file called "GJNZ.dat". 
...
for (i in seq_along(queries)) {
  ...
  prj[["V2_Ref_Final_report"]][[queries[i]]][["scenario"]] <- "Reference"
  prj[["V2_NZ70_Final_report"]][[queries[i]]][["scenario"]] <- "Net_Zero"
...
#emi <- writexl::write_xlsx(x = emi,"emi.xlsx")
#emi <- read_xlsx("emi.xlsx")
...
geom_area(data = emi %>% filter(year>2015),mapping=aes(x = year,y=value*3.67,fill=sector))+
  ...
  scale_fill_manual(values = my_custom_palette[2:6])+
...
ggplot()+
  geom_line(fe_tot,mapping=aes(x=year,y=value,color=scenario=linetype=scenario))
...
writexl::write_xlsx(trans_share_pass,"t1rans_share_pass.xlsx")
##########################
