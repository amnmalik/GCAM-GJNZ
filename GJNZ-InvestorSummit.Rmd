...
knitr::opts_chunk$set(
  ...
  cache = TRUE,
  fig.align = "center",
  fig.width = 8, # Increase figure width
  fig.height = 5, # Increase figure height
  out.width = "80%" # Ensure figures take enough horizontal space
)
...
queryFileInput <- "data/queries.xml"
...
ggplot(socioeco %>% filter(region %in% c("GJ", "TN", "KA", "MH"), category == "Pop", year > 2005, year < 2075, scenario == "Reference"), mapping = aes(x = year, y = value / 1e3, color = region)) +
  ...
  geom_label_repel(socioeco %>% filter(region %in% c("GJ", "TN", "KA", "MH"), category == "Pop", year == 2070, scenario == "Reference"),
...
ggplot(socioeco %>% filter(region %in% c("GJ", "TN", "KA", "MH"), category == "GDPPC", year > 2005, year < 2075, scenario == "Reference"),
  ...
  geom_line(
    data = socioeco %>% filter(region %in% c("GJ", "TN", "KA", "MH"), category == "GDPPC", year > 2005, year <= 2020, scenario == "Reference"),
    aes(x = year, y = value * 1000 * 1.98, color = region),
    linetype = "solid", size = 1.2
  ) +
  geom_point(shape = "circle", size = 2) +
  geom_line(
    data = socioeco %>% filter(region %in% c("GJ", "TN", "KA", "MH"), category == "GDPPC", year >= 2020, year < 2075, scenario == "Reference"),
    aes(x = year, y = value * 1000 * 1.98, color = region),
    linetype = "dashed", size = 1.2
  ) +
  geom_label_repel(socioeco %>% filter(region %in% c("GJ", "TN", "KA", "MH"), category == "GDPPC", year == 2070, scenario == "Reference"),
...
writexl::write_xlsx(x = emi, "output_data/disAggEmissions.xlsx")
...
writexl::write_xlsx(x = emi_tot, "output_data/AggEmissions.xlsx")
...
```{r}
gdp_gj <- getQuery(prj, "GDP by region") %>%
  mutate(category = "GDP") %>%
  filter(region == "GJ", year > 2010) %>%
...
writexl::write_xlsx(enint_gdp, "output_data/energy_intensity2.xlsx")
enint_gdp <- readxl::read_excel("output_data/energy_intensity2.xlsx")
...
elec <- getQuery(prj, "final energy consumption by aggregate sector and fuel") %>%
  filter(region == "GJ", input == "electricity", year > 2015) %>%
...
writexl::write_xlsx(elec, "output_data/elec_demand.xlsx")
...
# Correcttions to data due to calibration post-processing
elec_gen <- elec_gen %>%
  ...
  mutate(value = if_else(subsector == "gas" & value<0,0,value)) %>% 
  mutate(value = if_else(subsector == "coal" & value<0,0,value)) %>% 
  group_by(scenario, year) %>%
...
elec_gen_share <- elec_gen %>%
  select(-value, -output, -region) %>%
...
ggplot() +
  ...
  facet_wrap(~scenario) +
  scale_fill_manual(values = my_custom_palette[c(1, 2, 4, 6, 8, 10, 12, 14)]) +
...
###########################
ggplot() +
  geom_line(elec_gen_share %>% filter(year %in% c(2020, 2030, 2040, 2050), subsector %in% c("coal", "gas", "solar", "wind", "nuclear")), mapping = aes(x = year, y = percent, color = subsector, linetype = scenario, group = interaction(scenario, subsector)), linewidth = 1.2) +
  # geom_point(elec_gen_share %>% filter(year > 2015, year < 2055, subsector %in% c("coal", "gas", "solar", "wind", "nuclear")), mapping = aes(x = year, y = percent, color = subsector, group = interaction(scenario, subsector)), size = 3) +
  ...
  # facet_wrap(~scenario) +
  labs(y = "Share of electricity generation (%)", x = "Year") +
  scale_color_manual(values = my_custom_palette[c(2, 4, 6, 8, 10, 12)]) +
  theme(
    legend.position = "top",
  ) +
...
writexl::write_xlsx(elec_gen_share %>% filter(year > 2015, subsector %in% c("coal", "gas", "solar", "wind", "RE", "nuclear")), "output_data/elec_gen_share.xlsx")
##################################
```
...
```{r}
elec_cap <- read_excel("data/Cap_data_based_on22Jan25.xlsx",
  sheet = "Sheet1"
) %>%
  pivot_longer(2:10, names_to = "subsector", values_to = "value")

elec_cap_tot <- elec_cap %>%
  # filter(variable=="cumulative") %>%
  # mutate(year=as.numeric(year)) %>%
  group_by(year, scenario, Unit, variable) %>%
  summarise(value = sum(value))

elec_cap$scenario <- factor(elec_cap$scenario, levels = c("Reference", "Net_Zero"))


writexl::write_xlsx(elec_cap, path = "output_data/elec_cap.xlsx")
...
writexl::write_xlsx(fe %>% filter(year %in% c(2020, 2050, 2070)), "output_data/fe_absolute_shares.xlsx")
...
trans <- getQuery(prj, query = "transport service output by mode") %>%
  ...
  mutate(Shipping = `Ship Domestic` + `Ship International`) %>%
  pivot_longer(5:16, names_to = "mode", values_to = "value") %>%
  filter(!is.na(value))
...
writexl::write_xlsx(trans %>% filter(!mode %in% c("Ship International", "Ship Domestic", "Aviation")), "output_data/transport_demand.xlsx")


##########################
# Passenger and Freight
a <- ggplot() +
  geom_area(trans %>% filter(year > 2015, !mode %in% c("Ship International", "Ship Domestic", "Aviation"), scenario == "Reference"), mapping = aes(x = year, y = value / 1000, fill = mode, group = interaction(mode, scenario))) +
...
ggplot() +
  geom_area(
    data = trans_fuel %>% filter(year > 2015, year < 2075, input != "hydrogen"),
    mapping = aes(x = year, y = value, fill = input, group = interaction(input))
  ) +
  facet_wrap(~scenario) +
...
build <- getQuery(prj, "building final energy by service (Cooking) and fuel") %>%
  filter(region == "GJ", sector == "Cooking", year > 2010) %>%
  group_by(scenario, input, year) %>%
  summarise(value = sum(value))
...
yy <- ggplot(build4 %>% filter(year %in% c(2020, 2050, 2070), !sector %in% c("Heating")), mapping = aes(x = as.factor(year), y = percent, fill = sector)) +
...
ggsave("output_plots/BUILDING_service_share.png",
  device = "png", dpi = "print",
  width = 7, height = 5
)

xx / yy + plot_layout(guides = "collect") + plot_annotation(tag_levels = "a")

ggsave("output_plots/building_service_total_and_share.png",
  device = "png", dpi = "print",
  width = 9, height = 10
)
...
build6 <- getQuery(prj, "building final energy by fuel") %>%
  filter(region == "GJ") %>%
  group_by(scenario, year) %>%
  mutate(percent = (value / sum(value)) * 100)
...
a <- ggplot() +
  geom_area(
    data = industry %>% filter(year > 2015),
    mapping = aes(x = year, y = value, fill = input)
  ) +
  facet_wrap(~scenario) +
...
b <- ggplot(
  data = industry %>% filter(year %in% c(2020, 2050, 2070)),
  mapping = aes(x = as.factor(year), y = percent, fill = input)
) +
  geom_bar(stat = "identity") +
  facet_wrap(~scenario) +
  scale_fill_manual(values = my_custom_palette[c(2, 4, 6, 8, 10, 12)]) +
  labs(y = "Share of fuels (%)", x = "") +
  geom_text(aes(label = ifelse(percent > 5, paste0(round(percent, 0), "%"), "")),
    position = position_stack(vjust = 0.5),
...
ggsave("output_plots/combined_plot.png",
...
# making some post model run changes
agri <- read_excel("output_data/agri.xlsx") %>%
  group_by(scenario, year) %>%
  mutate(percentage = (value / sum(value)) * 100)
...
writexl::write_xlsx(new_scen, "output_data/ccs_scenario.xlsx")
...
writexl::write_xlsx(coal_gen, path = "output_data/coal_gen_inCCSscen.xlsx")
