...
prj <- loadProject(proj = "RV_GJNZ2.dat")
```
...
elec <- getQuery(prj,"final energy consumption by aggregate sector and fuel") %>% 
  ...
  group_by(scenario,sector,year) %>% 
    summarise(value=sum(value)) %>%  mutate(scenario="BAU_GJ") %>% 
  group_by(year) %>% 
  mutate(percent=value/sum(value),percent=percent*100)
...
ggplot()+
geom_area(fe %>% filter(year>2010,year<2075,scenario=="BAU_GJ"),mapping=aes(x=year,y=value,fill=input))+
...
ggplot()+
geom_area(trans %>% filter(year>2010,!mode %in% c("Ship Domestic","Ship International","Aviation")),mapping=aes(x=year,y=value/1000,fill=mode,group=interaction(mode,scenario)))+
...
floorspace <- getQuery(prj,"Building floorspace") %>% 
  filter(region=="GJ") %>% mutate(scenario="BAU_GJ") %>%  group_by(scenario, building) %>%
