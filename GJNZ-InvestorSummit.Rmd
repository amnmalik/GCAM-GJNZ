...
```{r}
prj <- mergeProjects("data/GJNZ_BAU_24June.dat","data/GJNZ_NZ70_24June.dat")
...
gdp_gj <- getQuery(prj,"GDP by region") %>% mutate(category="GDP") %>% filter(region=="GJ",year > 2000)                                                             #   scenario=="Revised_NZ70",
...
a <- ggplot(industry %>% filter(year == 2020,scenario=="Revised_BAU"),mapping = 
...
b <- ggplot(industry %>% filter(year == 2070,scenario=="Revised_BAU"),mapping = 
       aes(x = "", y = percent, fill = input, group = scenario)) +
  geom_col(color = "white", size = 1) +
  geom_text(data = . %>% filter(percent > 3),
    aes(label = paste0(round(percent, 0), "%")),
